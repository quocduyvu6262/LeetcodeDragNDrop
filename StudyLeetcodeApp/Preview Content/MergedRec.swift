...
struct OffsetMergedRectangles: Shape {
  ...
  func path(in rect: CGRect) -> Path {
    ...
    path.addCurve(
      to: CGPoint(x: bottomRect.maxX, y: bottomRect.minY),
      control1: CGPoint(x: topRect.maxX, y: topRect.maxY + connectionHeight * 0.5), // Pull right
      control2: CGPoint(x: bottomRect.maxX, y: bottomRect.minY - connectionHeight * 0.5)
    ) // Pull right
    ...
    path.addCurve(
      to: CGPoint(x: topRect.minX, y: topRect.maxY),
      control1: CGPoint(x: bottomRect.minX, y: bottomRect.minY - connectionHeight * 0.5), // Pull left
      control2: CGPoint(x: topRect.minX, y: topRect.maxY + connectionHeight * 0.5)
    ) // Pull left
