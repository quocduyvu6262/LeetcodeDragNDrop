...
struct LineRectsPreferenceKey: PreferenceKey {
  ...
  static func reduce(value: inout [Int: CGRect], nextValue: () -> [Int: CGRect]) {
    value.merge(nextValue()) { _, new in new } // Merge dictionaries
  }
}
...
struct CodeSnippet: View {
  ...
  init(code: String) {
    self.code = code
    lines = code.components(separatedBy: .newlines).map { line in
    ...
    width = lines.map { calculateSnippetWidth(text: $0) }.max() ?? calculateSnippetWidth(text: code)
  }
  ...
  var body: some View {
    if lines.count == 1 {
      ...
      .padding(.vertical, 8)
      .frame(width: width, height: calculateTotalHeight(), alignment: .leading)
...
extension CodeSnippet {
  private func singleLineView(_ line: String) -> some View {
    Text(line)
      ...
      .lineLimit(1)
      .padding(.leading, 5)
      .frame(width: width, height: Constants.snippetHeight, alignment: .leading)
      .background(
